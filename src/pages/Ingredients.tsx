...
export default function Ingredients() {
  ...
  const filteredIngredients = ingredients
    .filter(ingredient => {
      // Text search - includes name, brand, and category
      ...
      // Nutrition range filters
      const matchesCalories = (!calorieMin || ingredient.calories >= parseFloat(calorieMin)) &&
                              (!calorieMax || ingredient.calories <= parseFloat(calorieMax))
      const matchesProtein = (!proteinMin || ingredient.protein >= parseFloat(proteinMin)) &&
                             (!proteinMax || ingredient.protein <= parseFloat(proteinMax))
      const matchesCarbs = (!carbsMin || ingredient.carbs >= parseFloat(carbsMin)) &&
                           (!carbsMax || ingredient.carbs <= parseFloat(carbsMax))
      const matchesFat = (!fatMin || ingredient.fat >= parseFloat(fatMin)) &&
                         (!fatMax || ingredient.fat <= parseFloat(fatMax))
